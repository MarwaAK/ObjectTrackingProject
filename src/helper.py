...
class Rescale(object):
    ...
    Args:
        ...
        is made.
    """
    def __init__(self, output_size,kContextFactor=2,kScaleFactor=10):
        assert isinstance(output_size, (int, tuple))
        self.output_size = output_size
        self.kContextFactor = kContextFactor
        self.kScaleFactor = kScaleFactor
    ...
    def __call__(self, sample, opts):
        ...
        bbox = BoundingBox(bb[0], bb[1], bb[2], bb[3],kContextFactor=self.kContextFactor,kScaleFactor=self.kScaleFactor)
...
def shift_crop_training_sample(sample, bb_params,kContextFactor=2,kScaleFactor=10):
    """
    ...
    (rand_search_region, rand_search_location,
        edge_spacing_x, edge_spacing_y) = cropPadImage(bbox_curr_shift,
                                                       currimg,kContextFactor,kScaleFactor)
    bbox_curr_gt = BoundingBox(currbb[0], currbb[1], currbb[2], currbb[3])
...
def crop_sample(sample,kContextFactor=2,kScaleFactor=10):
    """
    ...
    opts = {}
    image, bb = sample['image'], sample['bb']
    orig_bbox = BoundingBox(bb[0], bb[1], bb[2], bb[3],kContextFactor=kContextFactor,kScaleFactor=kScaleFactor)
    (output_image, pad_image_location,
        edge_spacing_x, edge_spacing_y) = cropPadImage(orig_bbox, image,kContextFactor,kScaleFactor)
    new_bbox = BoundingBox(0, 0, 0, 0,kContextFactor=kContextFactor,kScaleFactor=kScaleFactor)
...
def cropPadImage(bbox_tight, image,kContextFactor=2,kScaleFactor=10):
    pad_image_location = computeCropPadImageLocation(bbox_tight, image,kContextFactor=kContextFactor,kScaleFactor=kScaleFactor)
...
def computeCropPadImageLocation(bbox_tight, image,kContextFactor=2,kScaleFactor=10):
    ...
    objPadImageLocation = BoundingBox(roi_left,
                                      roi_bottom,
                                      roi_left + roi_width,
                                      roi_bottom + roi_height,kContextFactor=kContextFactor,kScaleFactor=kScaleFactor)
