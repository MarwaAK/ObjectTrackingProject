...
from src.helper import NormalizeToTensor, Rescale, crop_sample, bgr2rgb
...
parser.add_argument('-d', '--data-directory',
                    default='../data/OTB/bag', type=str,
                    help='path to video frames')


class GOTURN:
    """Tester for OTB formatted sequences"""
    def __init__(self, root_dir, model_path, device,kContextFactor,kScaleFactor):
        self.root_dir = root_dir
        self.device = device
        self.transform = NormalizeToTensor()
        self.scale = Rescale((224, 224),kContextFactor=kContextFactor,kScaleFactor=kScaleFactor)
        ...
        frames = [f for f in frames if 'bmp' in f]
    ...
    def _get_sample(self, idx):
        ...
        prev_sample, opts_prev = crop_sample({'image': prev, 'bb': prevbb},kContextFactor=self.kContextFactor,kScaleFactor=self.kScaleFactor)
        curr_sample, opts_curr = crop_sample({'image': curr, 'bb': prevbb},kContextFactor=self.kContextFactor,kScaleFactor=self.kScaleFactor)
    ...
    def get_rect(self, sample):
        ...
        bb = bb[:, 0]
        bbox = BoundingBox(bb[0], bb[1], bb[2], bb[3],self.kContextFactor,self.kScaleFactor)
