...
from src.helper import (shift_crop_training_sample, crop_sample,
...
class TrainingSeq(Dataset):
    ...
    def _parse_data(self, root_dir, target_dir):
        ...
        x=[]
        y=[]
        folders = glob.glob('/content/drive/My Drive/CompVis_Project/pygoturn-master/data/OTB/*')
        for folder in folders :
          seq = glob.glob(folder+'/img/*.bmp')
          ann = open(folder+'/groundtruth_rect.txt')
        ...
        y = np.array(y)
        self.len = len(y)
        print('Training Seq dataset parsing done.')
        print('Total number of annotations in Training seq = %d' % (len(x)))
        return x, y
    ...
    def get_bb(self, ann):
        ...
        #bottom = max(float(ann[2]), float(ann[4]),
                   #  float(ann[6]), float(ann[8]))
        return [float(ann[0]), float(ann[1]), float(ann[0])+float(ann[2]), float(ann[1])+float(ann[3])]
